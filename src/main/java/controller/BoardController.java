...
public class BoardController extends HttpServlet {
	...
	protected void doPro(HttpServletRequest request, HttpServletResponse response)
		...
		case "/insert": // insert 기능
			...
			break;
		case "/edit": // 수정 화면을 보여줌 (1. 수정할 데이터를 먼저 끌어오기)
		...
		/*
		 * -둘 다 페이지를 이동한다. redirect: URL의 변화 O, 객체의 재사용 X (request, response 객체) DB에 변화가
		 * 생기는 요청에 사용 (글쓰기, 회원가입...) insert, update, delete 조심...
		 * 
		 * forward: URL의 변화 X(보안의 이점), 객체의 재사용 O (request, response 객체) 단순 조회(리스트 보기,
		 * 검색) select...
		 */
	...
	public String insertBoard(HttpServletRequest request) {
		/*
		 * request.getParameter("user_id"); request.getParameter("title");
	...
	public String updateBoard(HttpServletRequest request) {
		...
		try {
			BeanUtils.populate(b, request.getParameterMap());
			dao.updateBoard(b); //dao에서 만든 객체를 실행시켜주면 된다? 
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String deleteBoard(HttpServletRequest request) {
		int board_no = Integer.parseInt(request.getParameter("board_no")); // 일단 board_no 받아오기
